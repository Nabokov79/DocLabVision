...
import ru.nabokovsg.temlservice.services.subsection.SubsectionTemplateToProtocolService;
...
public class ProtocolSubsectionTemplateController {
    ...
    private final SubsectionTemplateToProtocolService service;

    @Operation(summary = "Добавление нового шаблона документа")
    @PostMapping
    public ResponseEntity<ProtocolTemplateDto> saveFromProtocolTemplate(
            @RequestBody @Valid
            @Parameter(description = "Данные шаблона титульного листа, заголовков") NewSubsectionTemplateToProtocolDto subsectionDto) {
        return ResponseEntity.ok().body(service.save(subsectionDto));
    }
}
